...
function resps = computeCleanTest(hs,p_rule,correct,incorrect,params,index_cache)
...
% resplit items
resps = [sum(logsumexp(item_p(:,1:length(correct)))) ...
  sum(logsumexp(item_p(:,length(incorrect)+1:end)))];

...
function resps = computeNoisyTest(hs,p_rule,correct,incorrect,params,index_cache)
...
for i = 1:length(items)
  % noise up item according to alpha
  if rand > params.alpha
    items{i} = all_strings{Randi(length(all_strings))};
    index_cache.items(i) = find(cellfun(@(x) all(items{i} == x),all_strings));
  end  
